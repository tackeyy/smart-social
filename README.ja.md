...
| プラン | 月額 | 年額 | 主な制限 |
|--------|:----:|:----:|---------|
| Free | ¥0 | — | AI 生成 10 件/月、スケジュール 5 件/月 |
| Pro | ¥4,980 | ¥47,800 | AI 生成 100 件/月、Xアカウント 3 件、Auto-plug/Evergreen 各 3 ルール |
| Business | ¥12,800 | ¥122,900 | AI 生成無制限、Xアカウント 10 件、チームメンバー 5 名 |

年払いは 2 ヶ月分無料（月額 × 10 ヶ月相当）。

## デプロイ

本番デプロイは `scripts/deploy.sh` を使います。マイグレーションの自動適用と Vercel デプロイを一括実行します。

```bash
# 事前: Supabase アクセストークンをシェル環境変数にセット
